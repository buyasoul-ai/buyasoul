...
const GENESIS_DIR = path.resolve(__dirname, 'src/genesis');
...
// Load executor (bridge test above loaded selection+orderGen; load executor now)
{
  ...
  const code5 = fs5.readFileSync(path5.join(GENESIS_DIR, 'rts-order-executor.js'), 'utf8');
...
console.log('\n=== Test 6: Minimap (RTS-6) ===');
{
  ...
  for (const name of ['rts-minimap.js', 'rts-production-palette.js']) {
    const code = fs6.readFileSync(path6.join(GENESIS_DIR, name), 'utf8');
